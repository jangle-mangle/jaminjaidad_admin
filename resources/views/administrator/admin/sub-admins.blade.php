                      ...
                      <div class="row">
                        ...
                        <div class="col-xl-3 col-lg-3 col-md-3 col-sm-12 col-xs-12">
                          <div class="form-group">
                            ...
                            <select class="form-control select2" id="buiness_type">
...
$(function(){
  $("#search-btn").on("click", function(evt){
        ...
        var city = $("#city").val();
        var buiness_type = $("#buiness_type").val();
        var status = $("#status").val();
       

        accomodationTable(vendor,city,buiness_type,status)
      });
  ...
  function accomodationTable(vendor='',city='',buiness_type='',status='') {
    ...
    $(document).on('click', '.approve', function(e) {
        ...
        var action = $(this).attr('name');
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              status_approve(id, action);
            } 
          })
      
    });

    const status_approve = (id = "", action = "") => {
      $.ajax({
              type: "post",
              url: "./vendor/approve",
              data: { 'id' :id , 'status': action, '_token' : '{{ csrf_token() }}' },
    ...
    $(document).on('click', '.reject', function(e) {
        ...
        var action = $(this).attr('name');
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              status_reject(id, action);
            } 
          })
      
    });

    const status_reject = (id = "", action = "") => {
      $.ajax({
              type: "post",
              url: "./vendor/reject",
              data: { 'id' :id , 'status': action, '_token' : '{{ csrf_token() }}' },
